  ...
  this.profileService.findColorsByIdVendedor(+this.vendedor_id, this.token).subscribe(
    (resposta: CorModel[]) => { // Certifique-se de especificar que a resposta é um array de CorModel
      ...
      if(resposta.length == 0){
          ...
          this.primaryColor = this.Cores[0].primary_color;
          this.secondaryColor =  this.Cores[0].secondary;
      }
  },
  (error) => {
...
atualizarCores(){
    ...
    const NewCor: CorModel = {
       id: this.Cores.length > 0 && this.Cores[0].id != null ? this.Cores[0].id : 0,
       vendedor: ''+this.vendedor_id,
       primary_color: ''+this.primaryColor,
       secondary: ''+this.secondaryColor
   };

   this.profileService.AtualizaCoresVendedor(NewCor, this.token).subscribe((resposta: CorModel) => {
    this.buscarCores();
}, () => {
